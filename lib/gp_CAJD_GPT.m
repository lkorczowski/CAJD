function [B,D,S,C,info,options] = gp_CAJD_GPT(X,C,options)
% [B,D,S,C,info,options] = gp_CAJD_GPT(X,C,options)
% Compute composite joint diagonalization of matrices X and C. 
%
% INPUTS
% -----------------
...
%   C : the product B'*C*B
%
% see also: gp_AJD_GPT, gp_BAJD_GPT
%
% *** History: 09-Fev-2016
...
F = size(C,3); % number of C matrices
...
% init info structure
info = struct('it',0,'conv',nan,'B',B,'D',D,'critOFFS',gp_critOFF(S),'critOFFC',gp_critOFF(C),'critA',gp_critMoAm(A,B),'critE',gp_critMoAm(E,D));
...
while it<itMax && conv>eps
    ...
    for i=1:N
        ...
        if  j~=i
            % get beta
            beta  = - (sum(S(i,j,:).*S(j,j,:)) + sum(Ctmp(i,j,:).*Ctmp(j,j,:)))/(sum(S(j,j,:).*S(j,j,:)) + sum(Ctmp(j,j,:).*Ctmp(j,j,:)));
            % get gamma
    ...
    info(it+1).B        = B;
    info(it+1).D        = D;
    info(it+1).critOFFS = gp_critOFF(S);
    info(it+1).critOFFC = gp_critOFF(Ctmp);
